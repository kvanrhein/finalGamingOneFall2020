...
int squareSize = 50;
 ...
 void draw() {
  ...
  if (p.x== k.x && p.y == k.y){
   ...
   // p.y = int(random(0, 12));
    gameStage = 3;
    level1Complete = new Level1Complete();
...
void buildWalls() {
  ...
  walls[w++] = new Wall(10, 1, false); 
  walls[w++] = new Wall(9, 5, false); 
  walls[w++] = new Wall(9, 6, false); 
  walls[w++] = new Wall(10, 5, false); 
  ...
  walls[w++] = new Wall(12, 9, true); 
  walls[w++] = new Wall(13, 9, true); 
}
}
